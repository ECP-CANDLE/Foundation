...
class PileH5Dataset(Dataset):
    ...
    def __init__(self, datapath, split, args):
        ...
        if zero: print(f"Found {nfiles} files under {split}.")
        for i in sorted(range(len(self.filenames)), reverse=True):
            ...
            if i%10 != 9 and split == 'test':
                popfile = self.filenames.pop(i)
        testfiles = h5py.File(self.filenames[0], 'r')['input_ids']
        tfshape = testfiles.shape
        self.args = args
        nfiles = len(self.filenames)
        nlines = 0
        ...
        if hasattr(args, 'seq_length'):
            ...
            self.window = args.seq_length
        self.max_window=tfshape[0]
        for fn in self.filenames:
            nlines += self.nperfile
        self.line_count = nlines
        try:
            self.mask_token = testfiles.attrs['mask_token']
            self.pad_token = testfiles.attrs['pad_token']
            self.unk_token = testfiles.attrs['unk_token']
        except:
            self.mask_token = 50258
            self.pad_token = 50257
            self.unk_token = 50256
        if zero:
