...
Since the theta system doesn't support pushd, we need to make some work arounds. `run.sh` builts deepspeed configuration, `node.sh` wraps up setting environment and launch deepspeed launcher in each sever, which is called from the cobalt script.
...
## node.sh
In each worker node, load modules, activate conda environment, set env vars, and call deepspeed launcher with master address and ds config (generated by run.sh).
