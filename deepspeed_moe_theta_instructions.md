...
conda create -p /projects/CSC249ADOA01/hsyoo/DS_MoE_NLG/conda_env python=3.8
...
git checkout 684c4733b1d44f6edfe4b190bf0cc55eb1ae1940
...
git checkout fa0735760a14a1d02df7949931e4caf6a33eeb0e
pip install -r requirements.txt
pip install .
```

# Prep Running
Check `/projects/CSC249ADOA01/hsyoo/DS_MoE_NLG/Megatron-DeepSpeed/examples/MoE` in theta, for the existing settings.

Since the theta system doesn't support pushd, we need to make some work arounds. `ds_pretrain_gpt_1.3B_MoE128.sh` builts deepspeed configuration, `node.sh` wraps up setting environment and launch deepspeed launcher in each sever, which is called from the cobalt script.

## pretrain.sh 
This is a cobalt script
```
#!/bin/bash -l
#COBALT -n 8
#COBALT -t 00:30:00
...
Generate deepspeed configuration file. This is called by pretrain.sh
```
#!/bin/bash
DIR=`pwd`
###############################################################################
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
    ...
    BASE_DATA_PATH=/projects/SuperBERT/hsyoo/pile-data
    VOCAB_PATH=${BASE_DATA_PATH}/gpt2-vocab.json
    MERGE_PATH=${BASE_DATA_PATH}/gpt2-merges.txt
    DATA_BLEND="1 ${BASE_DATA_PATH}/my-gpt2_text_document"
    DATA_BLEND="1 ${BASE_DATA_PATH}/pile_text_document"
fi
###############################################################################
data_options=" \
...
deepspeed_options=" \
		    --deepspeed \
		    --deepspeed_config ${DIR}/${config_json} \
...
run_cmd="${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options}"
echo ${run_cmd} > "exec_params.txt"
```


## node.sh
In each worker node, load modules, activate conda environment, set env vars, and call deepspeed launcher with master address and ds config.
